...
description: "新規作成または既存注入で dna_kernel を導入し、_workingspace/overview.md と docs/dna-kernel/ を起点に支援スケルトンを立ち上げる"
targets: ["*"]
---
...
既存構成を壊さず、`_workingspace/overview.md` と `docs/dna-kernel/` を起点に rulesync・uv・ワークスペースを整える。
...
- 既存の `_workingspace/overview.md` があり、ユーザーが通常の作業を依頼しているとき
...
- ソフトウェアプロジェクトでは、起動方法、開発方法、コンパイル方法、配布方法など利用者向けの手順を置く。
- 内部向けの目的、制約、作業方針は `_workingspace/overview.md` に置く。
...
7. 必要なら `_workingspace/overview.md` を作るか確認し、プロジェクト内容を聞く。

## 新規プロジェクト作成モード

### 1. _workingspace/overview.md の有無を確認する

プロジェクトルートで `_workingspace/overview.md` を探す。
...
- `_workingspace/overview.md` を作成するかユーザーに確認する。
- 作成しないと言われた場合は、導入作業を止め、通常の依頼として扱う。

確認文の例:

```text
このプロジェクトには _workingspace/overview.md がまだありません。プロジェクト概要の正本として作成しますか？
```

### 2. rulesync と uv の導入状況を確認する

`_workingspace/overview.md` を作成する了承を得たら、次を確認する。
...
### 5. _workingspace/overview.md を作成する

回答をもとに、`_workingspace/overview.md` を作成する。
初期テンプレート:
